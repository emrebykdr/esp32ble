...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _listenStreams() {
    ...
    _bleService.sensorStream.listen((value) {
  ...
  Future<void> _toggleScan() async {
    if (_isScanning) {
      ...
      setState(() => _isScanning = true); // önce butonu güncelle
      try {
        await _bleService.startScan();
      } catch (e) {
        debugPrint('Tarama hatası: $e');
      } finally {
        setState(() => _isScanning = false); // tarama bitince sıfırla
      }
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Bağlı cihaz kartı ve kontroller — bağlıysa göster
              if (_isConnected) ...[
                const SizedBox(height: 8),
                ConnectedCard(
                ...
                const SizedBox(height: 8),
                SensorCard(
                  distanceCm: _sensorValue,
                  onMeasure: _bleService.requestSensor,
                ),
              ],
            ],
          ),
